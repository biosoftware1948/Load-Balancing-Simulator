...
class S_D_HashLoadBalancer(LoadBalancer):
    def run_load_balancing():
        ...
        if not attr:
          ...
          os.flush()
          while not load_balancer.__JOB_QUEUE.isEmpty(): #just modulo the job object IDs onto the node IDs
            j = load_balancer.get_next_job()
            cluster.nodes[id(j) % cluster.node_count].assign_job(j)
        else:
          i = 0
          while not load_balancer.__JOB_QUEUE.isEmpty(): #just modulo the job source_dest attributes onto the node IDs
            j = load_balancer.get_next_job()
